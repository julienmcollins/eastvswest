...
test('POST /api/me/sync stores the fixture and embeds the whole leaderboard', async () => {
  ...
  // One request per month -- Jun, Jul, Aug -- and August's 202 rides need a second page. `now` is
  // 2026-08-31 here, so September is not asked for at all. `activities_scanned` above stays at the
  // fixture's 216 because it counts DISTINCT ids: adjacent months overlap by the +/-86400 s pad, so
  // a boundary ride comes back twice and must not be double-counted.
  assert.equal(res.json.pages_fetched, 4);
