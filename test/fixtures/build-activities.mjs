 ...
 *
 * Picker bounds assumed: 2026-06-01 .. 2026-08-31, i.e. the three months 2026-06, 2026-07 and
 * 2026-08 (see testConfig()).
 */
...
/**
 * The same counted set split BY MONTH, because every calendar month is its own competition
 * and no single query returns more than one of them.
 *
 * Derived here rather than in each test for the same reason the totals above are: a test that
 * hand-types "August is 202" stops being a check on the month filter and becomes a copy of
 * whatever the filter did on the day it was written. `sum` is asserted against
 * `counted_miles` in test/sync.test.js, which is what proves the months PARTITION the fixture
 * -- no ride counted twice, none lost between two boards.
 */
const byMonth = {};
for (const a of counted) {
  const month = a.start_date_local.slice(0, 7);
  const slot = (byMonth[month] ??= { records: 0, meters: 0, miles: 0 });
  slot.records += 1;
  slot.meters += a.distance;
}
for (const slot of Object.values(byMonth)) {
  slot.miles = Math.round((slot.meters / MILE) * 10) / 10;
}
...
const fixture = {
  ...
  /** The OUTER BOUNDS of the month picker, not one race's window. */
  ...
  expected: {
    ...
    /** `{ 'YYYY-MM': { records, meters, miles } }` -- one standalone competition each. */
    by_month: byMonth,
...
process.stdout.write(
  `wrote ${out}\n` +
    ...
    `  by month: ${fixture.months.map((m) => `${m}=${byMonth[m].miles}mi/${byMonth[m].records}`).join('  ')}\n`,
);
