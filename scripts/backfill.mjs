...
/**
 * Force a full re-sync of every athlete, oldest history included.
 *
 *   npm run backfill
 *   npm run backfill -- 12345678        # one athlete
 *   npm run backfill -- --dry-run       # list who WOULD be synced, spend no quota
 *
 * Why this exists. `computeSyncWindow` used to floor the fetch range at the first of the current
 * month whenever COMPETITION_START was in the future, so months before this one were never
 * downloaded. Widening the window fixes every FUTURE sync, but it cannot retroactively fetch what
 * was skipped: that needs one `mode:'full'` run per athlete against the widened range.
 *
 * It is not strictly required. `FULL_SYNC_INTERVAL_SECONDS` is a day, so auto mode picks 'full'
 * on each rider's next sync anyway and the backfill happens on its own within 24 hours. This
 * makes it immediate, and -- more usefully -- verifiable: the per-athlete `+added` column is the
 * evidence that the window fix actually recovered the missing rides.
 */
