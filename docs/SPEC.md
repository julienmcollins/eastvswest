...
server/strava/map.js             Pure: normalizeActivity, isCountedSportType, computeSyncMonths, parseRateLimitHeaders, localDateOf, nextQuarterHourMs, nextUtcMidnightMs, toBindable.
...
| POST | `/api/me/sync` | required | yes | `{}` or `{"mode":"incremental"\|"full","include_others":true}` (default: `full` if `now-last_full_sync_at>86400`, else `incremental`; `include_others` defaults false) | `{"ok":true,"mode","synced_at","activities_scanned","activities_counted","activities_added","activities_removed","pages_fetched","truncated",` `"miles":412.8,"others":{"attempted","synced","skipped","failed","rate_limited"}\|null,"leaderboard":{…identical to GET /api/leaderboard…}}` | 200, 401, 403 `insufficient_scope` (+`reauth_url`), 403 `strava_revoked` (+`reauth_url`), 409 `sync_in_progress` (+`retry_after_seconds`), 429 `rate_limited` (+`retry_after_seconds`,`scope:"local"\|"strava"`, `Retry-After` header, `Access-Control-Expose-Headers: Retry-After`), 502 `strava_unavailable` |
...
**5. Sync service.** Files: `server/strava/sync.js`, `test/sync.test.js`. Sequence: acquire `sync_state` lock (respecting `lock_expires_at`; a startup sweep flips stale `running` rows to `error`) → cooldown check (60 s) → `getValidAccessToken` → `getAthlete` (upsert name/avatar, normalizing a non-`https:` avatar to `NULL`) → `computeSyncMonths` → **one `fetchAllActivities` per month, oldest first** → normalize all (**store everything, filter at query time** — dropping non-bike rides at fetch time makes any future `ALLOWED_SPORT_TYPES` change require a full resync against a rate-limited API) → one `batch` of upserts (+ deletion reconciliation in **full** mode only, **per month**, gated on that month's own fetch being complete, untruncated and fully readable) → advance watermark/`last_full_sync_at` **only** when every planned month was reached and reconciled.
**EVERY MONTH IS FETCHED BY NAME**, one request each. It used to be one wide range whose floor was the earliest of {configured first month, current month, first month already holding rides} — and that third source is derived from the rows the fetch itself writes, so it could only widen to a month that ALREADY had data. A month missed once (which a `COMPETITION_START` in the future guarantees, by collapsing the floor onto the current month) was therefore unreachable by all three sources forever: re-syncing recomputed the identical range and recovered nothing, while still reporting `ok`. Asking per month makes "was July fetched" a question about a list rather than about arithmetic, removes the need for any separate backfill step, and takes truncation off the table (one calendar month cannot approach `STRAVA_MAX_PAGES` × `STRAVA_PAGE_SIZE` = 8000 rides). It also tightens reconciliation, which is now scoped to one month against that month's own fetch — so a month coming back short can neither suppress nor wrongly authorize deletions in another. **The cost, stated:** a full sync is one request per month (~9 for a Jan–September season) instead of 2. `SYNC_MAX_MONTHS` caps it at 24, `FULL_SYNC_INTERVAL_SECONDS` limits full mode to once a day per rider, and `mode:'incremental'` asks only from the watermark's month onward — normally one request.

Each month's window pads **±86400 s on both ends** — justified as covering **both** possible `before`/`after` semantics (`|UTC offset| ≤ 14 h < 24 h`), *not* as a consequence of the unverified "filters on `start_date` UTC" claim. The pad makes adjacent months overlap by two days, so a boundary ride is returned twice; rows are deduped on activity id and `activities_scanned` counts distinct ids, or the number would inflate with the size of the fetch. Each month also carries an **unpadded, half-open** reconcile range, always strictly inside the window that was sent, so `reconcile ⊆ fetch` holds by construction; for the current month it is additionally clamped to `now + pad`, without which a future-dated upload would be soft-deleted as "no longer reported". `mode:'full'` ignores the watermark entirely. **A watermark alone is provably wrong for a competition**: a bikepacking trip uploaded a week late, a Garmin sync failure, a rider flipping a 3-week-old ride from "Only You" to "Everyone", or a distance correction all have a `start_date` older than the watermark and would be missed forever; `/athlete/activities` has no `modified_after`. Full mode is one request per month, bounded by `SYNC_MAX_MONTHS`. Deletion reconciliation is equally load-bearing: without it a rider's deleted 340-mile GPS-glitch ride counts for their team for the rest of the competition. **Verify:** `node --test test/sync.test.js` against a real `:memory:` DB (the **only** place the boolean/`undefined` bind bugs surface) — fixture #4 (`trainer:true`) and #12 (`private:true`) upsert cleanly; the fixture with `timezone`/`type` absent round-trips as SQL NULLs; two syncs leave `COUNT(*)` unchanged; an edited distance updates in place and recomputes `local_date`; a `queue500` mid-pagination persists earlier rows and **leaves the watermark unmoved**; a full-mode sync with one id removed from the fake soft-deletes exactly that row; a truncated run soft-deletes nothing; a 500 in August still reconciles July's phantom row while leaving August's untouched; and a plain full sync against a backdated `COMPETITION_START` asks for every month from January to now, which is what removed the need for a backfill script.

**The roster sweep (`include_others`).** The board is shared, so a rider pressing Refresh wants everyone's current miles — but fanning out multiplies the only scarce resource in this app (Strava's 100 reads / 15 min) by the roster size. `syncOtherAthletes` in `server/strava/sync.js` therefore: syncs the **caller first** with their requested mode, so a rate limit halfway down the roster still leaves their press having worked; sweeps everyone else on **auto mode and never `force`**, so each teammate's own 60 s cooldown throttles it (five people pressing Refresh in one minute produce one sweep, not five) while `FULL_SYNC_INTERVAL_SECONDS` still gets them a daily full rescan; **never lets a teammate's failure throw** (a revoked grant or dead refresh token must not turn the caller's successful refresh into an error page); **stops on a Strava 429** rather than hitting it once per rider, since the block is global to the app; and skips riders with no team, revoked, disconnected or tokenless **without spending a request**. Sequential, never `Promise.all` — the rate-limit gate, the observed-429 block and the single-flight spacer are per-instance state on one shared client. It is **opt-in on the wire** because `public/app.js` also syncs automatically on every page load; fanning that out would make opening a tab a roster sweep. The response's `others` block is reported rather than silent: every non-`synced` count is a rider whose miles on the embedded board are older than the response implies, and `rate_limited: true` means the tail of the roster is stale — which is why `syncStatusMessage` in `public/app.js` refuses to say "Up to date." for it. **Verify:** `node --test test/sync.test.js test/routes-api.test.js test/frontend-contract.test.js` — a cooldown refusal counts as a skip and spends zero requests on a second sweep; a 429 stops after one rider instead of three; a teammate with undecryptable tokens yields `others.failed=1` and a **200** for the caller; and `syncStatusMessage` never renders `NaN` from a malformed wire field.
