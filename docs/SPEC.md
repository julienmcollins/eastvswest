...
.github/workflows/pages.yml      Deferred; text ready in "Deferred to deploy time".
```
...
1. **Domain strategy — decide before migrating, not during.** Put the site and the API on **one registrable domain**: `public/` on `www.example.com` (Pages custom domain via `public/CNAME`) and the API on `api.example.com`. This is the only design that keeps the session cookie `HttpOnly; Secure; SameSite=Lax` working unchanged. The alternative both critiques rejected — Pages on `user.github.io` + API on `workers.dev` — is broken by default browser policy, not by a config gap: the session cookie becomes a third-party cookie that Safari's ITP blocks unconditionally and Chrome/Firefox partition, so OAuth succeeds and every subsequent `/api/*` returns 401 forever with no CORS error and no 4xx to debug. `SameSite=None; Partitioned` does not fix it. A custom domain also gives the frontend a **dedicated origin**, which matters independently: `localStorage` is keyed per-origin with no path component, so on `user.github.io` any other project that account ever published — including a vendored third-party toy — can read the bearer token and impersonate any rider, including an admin.
...
5. **Bearer path (only if you cannot get a shared domain).** Flip the callback to 302 to `…#token=<opaque>`; `adoptTokenFromHash()` and the bearer branch of `resolveSession` already exist. Fragment, never a query param — fragments never reach server, proxy, or CDN logs. If you take this path, shorten session TTL to hours and gate admin actions behind re-auth.
6. **Cloudflare Workers + D1** (explicitly the target; **Netlify + file-backed SQLite is not viable** — ephemeral filesystem). `node:sqlite` does not exist on Workers. Swap `server/db/db.js` only: `all/get/run` map to `D1PreparedStatement`, `batch` to `D1.batch`. The migration files are already `wrangler d1 migrations apply` input. `loadConfig(env)` takes the Worker `env` binding instead of `process.loadEnvFile()`. Turso/libSQL over its plain-`fetch` HTTP API is the host-agnostic escape hatch and preserves zero dependencies. **Two known limitations to record in the deploy notes:** the in-process single-flight refresh mutex evaporates per isolate — the `token_version` CAS is what actually protects you there; and any post-response work must go through `ctx.waitUntil()`, which is why nothing depends on a fire-and-forget sync.
7. **Delete `server/http/static.js`** (one file) and `server/index.js`'s listen path; Pages serves `public/` directly. Add `public/404.html`.
